...
if not pending_approvals:
    ...
    st.info("When your team members submit feedback requests, they will appear here for your approval.")
else:
    st.write(f"You have **{len(pending_approvals)}** nomination(s) pending your approval:")
    ...
    for i, approval in enumerate(pending_approvals, 1):
        ...
        with st.container():
            ...
            with col1:
                ...
                st.write(f"**Reviewer Details:** {reviewer_vertical} - {reviewer_designation}")
                st.write(f"**Relationship Type:** {relationship_type.replace('_', ' ').title()}")
            ...
            with col2:
                ...
                with col_approve:
                    if st.button(f"✅ Approve", key=f"approve_{request_id}", type="primary"):
                        if approve_reject_feedback_request(request_id, user_id, 'approve'):
                ...
                if st.session_state.get(f'show_reject_form_{request_id}', False):
                    with st.form(f"reject_form_{request_id}"):
                        ...
                        with col_submit:
                            submit_rejection = st.form_submit_button("Submit Rejection")
                        with col_cancel:
                            cancel_rejection = st.form_submit_button("Cancel")
                        
                        if submit_rejection:
                            if rejection_reason.strip():
                                if approve_reject_feedback_request(request_id, user_id, 'reject', rejection_reason):
...
with st.expander("📋 What to consider when approving nominations"):
    st.write("""
...
st.info("ℹ️ **Note:** Only manager-level employees and above can request feedback from external stakeholders. The system automatically enforces this rule.")

if pending_approvals:
    st.warning("⏰ **Action Required:** Please review and approve/reject these nominations promptly to keep the feedback process on track.")